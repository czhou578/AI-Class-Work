...
class q_learner():
    def __init__(self, alpha, epsilon, gamma, nfirst, state_cardinality):
        ...
        for i in range(state_cardinality[0]):
          for j in range(state_cardinality[1]):
            for k in range(state_cardinality[2]):
              for l in range(state_cardinality[3]):
                for m in range(state_cardinality[4]):
                  qTable['{}{}{}{}{}'.format(i, j, k, l, m)] = {-1: 0, 0: 0, 1: 0}
                  nTable['{}{}{}{}{}'.format(i, j, k, l, m)] = {-1: 0, 0: 0, 1: 0}
    ...
    def report_exploration_counts(self, state):
        ...
        for action in actions:
          explored_count.append(self.NTable[key][action])
    ...
    def choose_unexplored_action(self, state):
        ...
        # print("nfirst", self.nfirst)
        # print("explored_count", explored_count)


        # count_less_nFirst = []
        # for idx in range(len(explored_count)):
        #   if explored_count[idx] < self.nfirst:
        #     if idx == 0:
        #       count_less_nFirst.append(-1)
        #     elif idx == 1:
        #       count_less_nFirst.append(0)
        #     elif idx == 2:
        #       count_less_nFirst.append(1)    

        # print(count_less_nFirst)    

        # if len(count_less_nFirst) != 0:
        #   random_action = random.randint(count_less_nFirst[0], count_less_nFirst[len(count_less_nFirst) - 1])
        #   # random_action = random.randint(count_less_nFirst[0], count_less_nFirst[len(count_less_nFirst) - 1])
        #   # random_action = random.choice(count_less_nFirst)
        #   # random_action = np.random.choice(count_less_nFirst, size=1)[0]
        #   random_action = np.random.choice(count_less_nFirst)


        #   self.NTable[state[0]][state[1]][state[2]][state[3]][state[4]][random_action] += 1
        #   # self.NTable[key][random_action] += 1
        ...
        # print("count_less_nFirst", count_less_nFirst)

        if len(count_less_nFirst) != 0:
          random_action = np.random.choice(count_less_nFirst)
          # random_action = random.randint(count_less_nFirst[0], count_less_nFirst[len(count_less_nFirst) - 1]) - 1
          self.NTable[key][random_action] += 1
          return random_action
    ...
    def report_q(self, state):
        ...
        for action in actions:
          qValues.append(self.qTable[key][action])
    ...
    def q_local(self, reward, newstate):
        ...
        '''
        # actions_of_state = self.qTable[state[0]][state[1]][state[2]][state[3]][state[4]]
        # max_action = max(actions_of_state)

        # q_local = reward + self.gamma * actions_of_state.index(max_action)



        key = ''.join(map(str, newstate))
        print("self.values()", self.qTable[key].values())
        q_local = reward + self.gamma * max(self.qTable[key].values())
        print("from q_local", max(self.qTable[key].values()))
    ...
    def learn(self, state, action, reward, newstate):
        ...
        # if action == -1:
        #   state_action_entry = self.qTable[state[0]][state[1]][state[2]][state[3]][state[4]][action]
        #   state_action_entry = state_action_entry + self.alpha * (self.q_local(reward, newstate) - state_action_entry)
        ...
        self.qTable[key][action] = self.qTable[key][action] + self.alpha * (self.q_local(reward, newstate) - self.qTable[key][action]) 
    ...
    def exploit(self, state):
        ...
        # max_key = actions.index(highest_q_value)
        ...
        max_key = max(actions, key=lambda k:actions[k])
        max_value = actions[max_key]
    ...
    def act(self, state):
        ...
        #       # random_number = random.choices([-1, 0, 1], k=1)[0]
        ...
        key = ''.join(map(str, state))
        actions = self.NTable[key]

        fewer_then_nfirst = []

        for key, value in actions.items():
          if value < self.nfirst:
            fewer_then_nfirst.append(key)
