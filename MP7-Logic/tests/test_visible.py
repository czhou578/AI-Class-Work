...
class TestStep(unittest.TestCase):
    ...
    def test_standardization(self):
        ...
        standardized, variables = submitted.standardize_variables(
            world['rules'])
        vars = list(variables)
        usedvars = []
        for (ruleid, rule) in world['rules'].items():
            ...
            hyp = [standardized[ruleid]['consequent']] + \
                list(standardized[ruleid]['antecedents'])
    ...
    @weight(12.5)
    def test_unify(self):
        unification, subs = submitted.unify(['x', 'eats', 'y', False], [
                                            'a', 'eats', 'b', False], ['x', 'y', 'a', 'b'])
        ...
        unification, subs = submitted.unify(['bobcat', 'eats', 'y', True], [
                                            'a', 'eats', 'squirrel', True], ['x', 'y', 'a', 'b'])
        ...
        self.assertEqual(tuple(sorted(tsubs.items())), tuple(sorted(subs.items())),
                         '''subs should be %s, not %s''' % (tsubs, subs))
        unification, subs = submitted.unify(['x', 'eats', 'x', True], [
                                            'a', 'eats', 'bobcat', True], ['x', 'y', 'a', 'b'])
        ...
        self.assertEqual(tuple(sorted(tsubs.items())), tuple(sorted(subs.items())),
                         '''subs should be %s, not %s''' % (tsubs, subs))
        unification, subs = submitted.unify(['a', 'eats', 'bobcat', True], [
                                            'x', 'eats', 'x', True], ['x', 'y', 'a', 'b'])
        tunify = ['bobcat', 'eats', 'bobcat', True]
        tsubs = {'a': 'x', 'x': 'bobcat'}
        self.assertEqual(tuple(tunify), tuple(unification),
                         '''unification should be %s, not %s''' % (tunify, unification))
    ...
    def test_apply(self):
        ...
        asets = [tuple(sorted(tuple(x) for x in applications[i]
                       ['antecedents'])) for i in range(2)]
    ...
    def test_backward_chain(self):
        ...
        for (qid, question) in world['questions'].items():
            proof = submitted.backward_chain(
                question['query'], world['rules'], world['variables'])
