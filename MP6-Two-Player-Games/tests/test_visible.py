...
def recursive_tree_test(testcase, label, ref, hyp, codelist):
    ...
    for k in hyp.keys():
        msg = label+' moveTree'+codelist+' contains ' + \
            k+' but, that should have been pruned away.'
        testcase.assertIn(k, ref, msg)
        recursive_tree_test(
            testcase, label, ref[k], hyp[k], codelist+'['+k+']')


def general_test(testcase, app, label, searchfunc, depth, breadth=None):
    if breadth:
        chooser = nonrandomChoice()
        hypVal, hypList, hypTree = searchfunc(
            app.side, app.board, app.flags, depth, breadth, chooser)
    else:
        hypVal, hypList, hypTree = searchfunc(
            app.side, app.board, app.flags, depth)
...
class grading_tests(unittest.TestCase):
    ...
    #     general_test(self, load_game(0), 'minimax_game%d_depth%d' %
    #                  (0, 1), submitted.minimax, 1)

    # @weight(7)
    # def test_minimax02(self):
    #     general_test(self, load_game(0), 'minimax_game%d_depth%d' %
    #                  (0, 2), submitted.minimax, 2)
    # @weight(7)
    # def test_minimax11(self):
    #     general_test(self, load_game(1), 'minimax_game%d_depth%d' %
    #                  (1, 1), submitted.minimax, 1)

    # @weight(7)
    # def test_minimax12(self):
    #     general_test(self, load_game(1), 'minimax_game%d_depth%d' %
    #                  (1, 2), submitted.minimax, 2)
    # @weight(7)
    # def test_alphabeta02(self):
    #     general_test(self, load_game(0), 'alphabeta_game%d_depth%d' %
    #                  (0, 2), submitted.alphabeta, 2)

    @weight(7)
    def test_alphabeta12(self):
        general_test(self, load_game(1), 'alphabeta_game%d_depth%d' %
                     (1, 2), submitted.alphabeta, 2)

    @weight(7)
    def test_alphabeta13(self):
        general_test(self, load_game(1), 'alphabeta_game%d_depth%d' %
                     (1, 3), submitted.alphabeta, 3)
