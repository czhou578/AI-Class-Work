...
def minimax(side, board, flags, depth):
    ...
    if (depth == 0 or len(moves) <= 0):
        return (evaluate(board), [], {})  # used to possibly update minValue

    if (side == True):  # black
        minValue = float("inf")

        for move in moves:
            encodedMove = encode(move[0], move[1], move[2])
            newside, newboard, newflags = makeMove(
                side, board, move[0], move[1], flags, move[2])

            eval = minimax(newside, newboard, newflags, depth - 1)
            moveTree[encodedMove] = eval[2]

            if (minValue > eval[0]):
                moveList = []
                eval[1].insert(0, move)
                moveList.insert(0, eval[1])
            minValue = min(minValue, eval[0])

        return (minValue, moveList[0], moveTree)
    else:  # white
        maxValue = float("-inf")
        ...
        for move in moves:
            ...
            if (maxValue < eval[0]):
                ...
                eval[1].insert(0, move)
                moveList.insert(0, eval[1])
            maxValue = max(maxValue, eval[0])

        return (maxValue, moveList[0], moveTree)
...
def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if (side == True):  # black
        ...
        for move in moves:
            ...
            eval = alphabeta(newside, newboard, newflags,
                             depth - 1, alpha, beta)
            if (minValue > eval[0]):
                moveList = []
                eval[1].insert(0, move)
